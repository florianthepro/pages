...
function render_settings(array $u): void {
    ...
    echo $row('?app=settings&view=account', 'user', '#4d7ea8', 'Konto');
    echo $row('?app=settings&view=appearance', 'sun', '#b3893f', 'Aussehen', $u['theme'] === 'dark' ? 'Dunkel' : 'Hell');
    echo $row('?app=settings&view=connections', 'mail', '#4a8ca0', 'Verbindungen', (string) (count(mail_accounts($u)) + count(nx_sync_accounts($u))));
    echo '</div>';
    ...
    if ($u['role'] === 'admin') {
        ...
        echo $row('?app=settings&view=system', 'db', '#4a9d6f', 'System');
        echo '</div>';
    }
