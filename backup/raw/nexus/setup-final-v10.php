...
function layout_head(array $user, string $activeApp): void {
    ...
    foreach (nx_groups() as $grp) {
        ...
        foreach ($items as $id) {
            ...
            if ($id === 'admin') {
                ...
                if ($open > 0) {
                    $badge = '<span class="rbadge warn">' . $open . '</span>';
                }
            }
    ...
    echo '<a class="ravatar' . ($activeApp === 'settings' ? ' active' : '') . '" href="' . url('settings') . '" title="Profil">'
        . '<span class="avatar">' . h(strtoupper(substr($user['display_name'] ?: $user['username'], 0, 1))) . '</span></a>';
