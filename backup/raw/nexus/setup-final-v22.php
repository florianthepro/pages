...
function view_auth(string $mode, ?string $err = null): void {
    sec_headers();
    $firstUser = user_count() === 0;
    echo '<!doctype html><html lang="' . h(nx_lang()) . '" data-theme="dark"><head><meta charset="utf-8">';
...
function render_waiting(array $u): void {
    ...
    echo '<!doctype html><html lang="' . h(nx_lang()) . '" data-theme="' . h($u['theme'] ?: 'dark') . '"><head>';
    echo '<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">';
...
function handle_settings(array $u, string $action): void {
    ...
    switch ($action) {
        ...
        case 'appearance':
            ...
            db_run('UPDATE users SET theme=?, accent=?, lang=? WHERE id=?', [$theme, $accent, $lang, $u['id']]);
            $_SESSION['lang'] = $lang;
...
function render_settings(array $u): void {
    ...
    if ($view === 'appearance') {
        ...
        echo '<div class="field"><label>' . h(t('set_language')) . '</label><select name="lang">';
        foreach (nx_langs() as $code => $lname) {
            echo '<option value="' . $code . '" ' . (nx_lang() === $code ? 'selected' : '') . '>' . h($lname) . '</option>';
        }
        echo '</select></div>';
