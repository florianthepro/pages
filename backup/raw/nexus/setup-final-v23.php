...
function render_store(array $u): void {
    ...
    $card = static function (string $id, string $name, string $desc, string $color, string $ico, bool $on, string $grp) use ($btn): string {
        $out  = '<div class="ccard wacard' . ($on ? ' installed' : '') . '" data-grp="' . $grp . '" data-name="' . h(mb_strtolower($name)) . '"><div class="chead">';
    ...
    // Kurze Erklaerung, wie das Hinzufuegen funktioniert
    echo '<div class="store-intro">' . icon('grid', 18) . '<div><strong>So fügst du Apps hinzu</strong>'
        . '<p>Tippe bei einer App auf <b>+ Hinzufügen</b> – sie erscheint danach direkt auf deiner Startseite. '
        . 'Zum Entfernen auf <b>✓ Hinzugefügt</b> tippen.</p></div></div>';
