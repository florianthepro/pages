...
function asset_css(): string {
    return <<<'NXCSS'
/* Nexus v3 – ruhiges, weiches Interface. Systemfonts, viel Luft,
   runde Flächen, dezente Tiefe. Rein visuell, keine Logikänderung. */
:root{
  --accent:#5b93d6;
...
.sidebar{background:var(--panel);border-right:1px solid var(--line);padding:18px 14px;display:flex;flex-direction:column;gap:2px;position:sticky;top:0;height:100vh}
...
/* Main */
.main{padding:26px 34px 60px;max-width:1240px;width:100%}
.topbar{display:flex;align-items:center;gap:14px;margin-bottom:24px}
.topbar h1{font-size:21px;font-weight:650;letter-spacing:-.3px}
...
/* Modal */
.modal{position:fixed;inset:0;background:rgba(0,0,0,.5);backdrop-filter:blur(2px);display:none;place-items:center;z-index:50;padding:20px}
.modal.open{display:grid}
.modal .box{background:var(--panel);border:1px solid var(--line);border-radius:18px;padding:24px;width:100%;max-width:500px;box-shadow:var(--shadow);max-height:90vh;overflow:auto;position:relative}
