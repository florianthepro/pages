...
function nx_webapps(): array {
    return [
        'youtube'      => ['name'=>'YouTube','cat'=>'Video','embed'=>true,'kind'=>'player','frame'=>'https://www.youtube-nocookie.com','url'=>''],
        ...
        'twitch'         => ['name'=>'Twitch','cat'=>'Video','embed'=>false,'url'=>'https://www.twitch.tv'],
        'vimeo'          => ['name'=>'Vimeo','cat'=>'Video','embed'=>false,'url'=>'https://vimeo.com'],
        ...
        'spotify'        => ['name'=>'Spotify','cat'=>'Musik','embed'=>false,'url'=>'https://open.spotify.com'],
        'soundcloud'     => ['name'=>'SoundCloud','cat'=>'Musik','embed'=>false,'url'=>'https://soundcloud.com'],
        ...
        'gmaps'          => ['name'=>'Google Maps','cat'=>'Tools','embed'=>false,'url'=>'https://maps.google.com'],
...
function render_web(array $u): void {
    ...
    if (($w['kind'] ?? '') === 'player') {
        // YouTube-Player: Link/ID eingeben -> eingebettet abspielen
        $v = trim(param('v'));
        $vid = '';
        if ($v !== '' && preg_match('#(?:v=|youtu\.be/|embed/|shorts/)?([A-Za-z0-9_-]{11})#', $v, $m)) {
            $vid = $m[1];
        }
        echo '<div class="panel" style="max-width:820px">';
        echo '<form method="get" style="display:flex;gap:8px;margin-bottom:12px"><input type="hidden" name="app" value="web"><input type="hidden" name="id" value="' . h($wid) . '">';
        echo '<input class="input" name="v" placeholder="YouTube-Link oder Video-ID einfügen" value="' . h($v) . '">';
        echo '<button class="btn">Abspielen</button></form>';
        if ($vid !== '') {
            echo '<div class="embed-16x9"><iframe src="' . h($w['frame']) . '/embed/' . h($vid) . '" allow="accelerometer;autoplay;clipboard-write;encrypted-media;gyroscope;picture-in-picture" allowfullscreen></iframe></div>';
        } else {
            echo '<div class="empty">' . icon('video', 40) . '<h3>Video einfügen</h3><p>YouTube-Link oder Video-ID oben eingeben.</p></div>';
        }
        echo '</div>';
        return;
    }
    // Reine Einbettung (Karte/Wikipedia)
    echo '<div class="embed-full"><iframe src="' . h($w['url']) . '" allowfullscreen referrerpolicy="no-referrer"></iframe></div>';
}
