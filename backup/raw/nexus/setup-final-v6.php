...
function nx_apps(): array {
    return [
        ...
        'admin'     => ['name'=>'Verwaltung',   'desc'=>'Nutzer, Freischaltung & Quota',  'icon'=>'shield',   'color'=>'#7a828e', 'tile'=>false, 'min'=>'admin'],
...
/* Shell: schmale Icon-Leiste links, Inhalt daneben – auf jedem Gerät
   gleich, kein verstecktes Menü. */
.shell{display:grid;grid-template-columns:64px 1fr;min-height:100vh;min-height:100dvh}
.rail{background:var(--panel);display:flex;flex-direction:column;align-items:center;gap:4px;position:sticky;top:0;height:100vh;height:100dvh;padding:12px 0 max(12px,var(--safe-b));padding-left:var(--safe-l);overflow-y:auto;overscroll-behavior:contain}
.rlogo{width:38px;height:38px;border-radius:11px;display:grid;place-items:center;color:#fff;background:var(--accent);box-shadow:var(--shadow-sm);margin-bottom:8px;flex:none}
.ricons{display:flex;flex-direction:column;align-items:center;gap:4px;flex:1;width:100%}
.rsep{width:22px;height:1px;background:var(--line);margin:5px 0;flex:none}
.ric{position:relative;width:42px;height:42px;border-radius:12px;display:grid;place-items:center;color:var(--muted);transition:background .12s,color .12s;flex:none}
.ric:hover{background:var(--panel2);color:var(--txt)}
.ric.active{background:var(--accent-soft);color:var(--accent)}
.rbadge{position:absolute;top:3px;right:1px;background:var(--accent);color:#fff;font-size:9.5px;font-weight:700;min-width:15px;height:15px;border-radius:8px;display:grid;place-items:center;padding:0 4px;font-family:var(--mono)}
.rbadge.warn{background:var(--warn)}
.ravatar{position:relative;margin-top:6px;border-radius:12px;padding:4px;flex:none}
.ravatar:hover{background:var(--panel2)}
.ravatar.active{background:var(--accent-soft)}
...
.ravatar .avatar{border-radius:10px}
...
.input,textarea,select{width:100%;min-width:0;max-width:100%;padding:10px 13px;border-radius:11px;border:1px solid transparent;background:var(--bg2);color:var(--txt);transition:border-color .12s,box-shadow .12s,background .12s;outline:none;font-size:13.5px}
/* native Datum/Zeit-Felder dürfen schrumpfen -> laufen nie über den Rand */
...
.cal{display:grid;grid-template-columns:repeat(7,1fr);gap:2px;background:var(--panel);border-radius:var(--radius);padding:10px;box-shadow:var(--shadow-sm)}
...
.cal .cell{background:transparent;border:0;border-radius:var(--radius-sm);min-height:92px;padding:5px;display:flex;flex-direction:column;gap:3px;transition:background .12s;cursor:pointer}
.cal .cell:hover{background:var(--panel2)}
.cal .cell.out{opacity:.3}
.cal .cell .num{font-size:12px;font-weight:600;color:var(--muted);font-family:var(--mono);width:24px;height:24px;display:grid;place-items:center;border-radius:99px}
...
// Aufgaben
function newTask(){const b=document.getElementById('taskModal');if(!b)return;const f=b.querySelector('form');if(f)f.reset();openModal('taskModal');setTimeout(()=>f&&f.title.focus(),50);}
...
// Kontakte
function newContact(){const f=document.getElementById('contactForm');if(!f)return;f.reset();f.id.value='';f.me.value='';document.getElementById('contactModalTitle').textContent='Neuer Kontakt';openModal('contactModal');}
function editContact(el){const d=el.dataset,f=document.getElementById('contactForm');f.id.value=d.id;f.name.value=d.name;f.email.value=d.email;f.phone.value=d.phone;f.birthday.value=d.birthday||'';f.address.value=d.address||'';f.note.value=d.note;f.me.value=d.me||'';document.getElementById('contactModalTitle').textContent=d.me?'Meine Karte bearbeiten':'Kontakt bearbeiten';openModal('contactModal');}
function newMyCard(name,email){const f=document.getElementById('contactForm');if(!f)return;f.reset();f.id.value='';f.me.value='1';f.name.value=name||'';f.email.value=email||'';document.getElementById('contactModalTitle').textContent='Meine Karte anlegen';openModal('contactModal');}
...
function layout_head(array $user, string $activeApp): void {
    ...
    /* Schmale Icon-Leiste links: Logo -> Start, App-Icons, unten der
       Avatar als Zugang zum Profil (dort: Meine Karte, Einstellungen,
       Speicher und fuer Admins die Verwaltung). */
    echo '<aside class="rail">';
    echo '<a class="rlogo" href="' . url('home') . '" title="' . NX_NAME . '">' . logo_mark(20) . '</a>';
    echo '<nav class="ricons">';
    $firstGroup = true;
    foreach (nx_groups() as $grp) {
        $items = [];
        ...
        foreach ($items as $id) {
            ...
            echo '<a class="ric' . ($id === $activeApp ? ' active' : '') . '" href="' . url($id) . '" title="' . h($a['name']) . '">'
                . icon($a['icon'], 21) . $badge . '</a>';
        }
    }
    echo '</nav>';
    $pbadge = '';
    if ($user['role'] === 'admin') {
        ...
        if ($open > 0) {
            $pbadge = '<span class="rbadge warn">' . $open . '</span>';
        }
    }
    echo '<a class="ravatar' . ($activeApp === 'settings' ? ' active' : '') . '" href="' . url('settings') . '" title="Profil">'
        . '<span class="avatar">' . h(strtoupper(substr($user['display_name'] ?: $user['username'], 0, 1))) . '</span>' . $pbadge . '</a>';
...
function render_home(array $u): void {
    layout_topbar('Start', strftime_de());
    ...
    /* 1) Lesezeichen: das Herz der Startseite – Links direkt hier
       anlegen, öffnen und löschen. */
    if (can_access($u, 'bookmarks')) {
        echo '<div class="section-h">' . icon('link') . ' Lesezeichen</div><div class="grid bmgrid">';
        foreach (db_all('SELECT * FROM bookmarks WHERE user_id=? ORDER BY position,id', [$u['id']]) as $row) {
            $b = dec_row($row['enc'], mk());
            if (!$b) {
                continue;
            }
            echo '<div class="bmtile" style="--tc:' . h($b['color'] ?? '#4d7ea8') . '">';
            echo '<a class="bmgo" href="' . h($b['url'] ?? '#') . '" target="_blank" rel="noopener">';
            echo '<span class="tico">' . icon('link', 15) . '</span>';
            echo '<span class="bmtxt"><strong>' . h($b['title'] ?? '') . '</strong><small>'
                . h(preg_replace('#^https?://(www\.)?#', '', (string) ($b['url'] ?? ''))) . '</small></span></a>';
            echo '<a class="bmdel" title="Löschen" href="?app=bookmarks&action=del&id=' . $row['id'] . '&_csrf=' . csrf_token()
                . '" onclick="return confirm(\'Lesezeichen löschen?\')">&times;</a>';
            echo '</div>';
        }
        echo '<div class="bmtile add" onclick="openModal(\'bmModal\')"><span class="tico">' . icon('plus', 15) . '</span>'
            . '<span class="bmtxt"><strong>Link hinzufügen</strong></span></div>';
        echo '</div>';
    ...
    echo '<div class="section-h">' . icon('grid') . ' Apps</div><div class="grid appgrid">';
    foreach (nx_apps() as $id => $a) {
        if (empty($a['tile']) || !can_access($u, $id)) {
            continue;
        }
        echo '<a class="app-chip" style="--tc:' . h($a['color']) . '" href="' . url($id) . '">';
        echo '<span class="tico">' . icon($a['icon'], 17) . '</span><span>' . h($a['name']) . '</span></a>';
    }
    echo '</div>';
}
...
function render_tasks(array $u): void {
    ...
    layout_topbar('Aufgaben', count($open) . ' offen',
        '<button class="btn" onclick="newTask()">' . icon('plus') . ' Aufgabe</button>');

    // Eingabe als Overlay statt Inline-Leiste -> saubere, gleich breite Felder
    echo '<div class="modal" id="taskModal"><div class="box">';
    echo '<span class="modal-x" onclick="closeModal(\'taskModal\')">&times;</span><h3>Neue Aufgabe</h3>';
    echo '<form method="post" action="?app=tasks&action=add">' . csrf_field();
    echo '<div class="field"><label>Aufgabe</label><input class="input" name="title" required></div>';
...
function render_contacts(array $u): void {
    ...
    // "Meine Karte" (eigener Kontakt) von den übrigen trennen
    $mine = null;
    $others = [];
    foreach ($rows as $c) {
    ...
    $card = static function (array $c, bool $isMe): string {
        ...
        $out .= '<div class="chead"><div class="avatar">' . h(strtoupper(mb_substr((string) ($c['name'] ?? '?'), 0, 1))) . '</div>';
        $out .= '<div class="cname"><strong>' . h($c['name'] ?? '') . '</strong>';
        if ($isMe) {
            $out .= '<small>Meine Karte</small>';
        }
        $out .= '</div><div class="cacts">';
        $out .= '<a href="#" title="Bearbeiten" onclick="editContact(this);return false"'
            . ' data-id="' . $c['id'] . '" data-name="' . h($c['name'] ?? '') . '" data-email="' . h($c['email'] ?? '')
            . '" data-phone="' . h($c['phone'] ?? '') . '" data-birthday="' . h($c['birthday'] ?? '')
            . '" data-address="' . h($c['address'] ?? '') . '" data-note="' . h($c['note'] ?? '')
            . '" data-me="' . ((int) ($c['me'] ?? 0) === 1 ? '1' : '') . '">' . icon('edit', 15) . '</a>';
        $out .= '<a href="?app=contacts&action=del&id=' . $c['id'] . '&_csrf=' . csrf_token()
            . '" title="Löschen" class="del" onclick="return confirm(\'Kontakt löschen?\')">' . icon('trash', 15) . '</a>';
        $out .= '</div></div><div class="cbody">';
        if (($c['email'] ?? '') !== '') {
    ...
    // Eigene Karte: vorhandene zeigen oder zum Anlegen einladen
    echo '<div class="section-h">' . icon('user') . ' Meine Karte</div>';
    if ($mine !== null) {
        echo '<div class="cgrid">' . $card($mine, true) . '</div>';
    } else {
        $own = $u['display_name'] ?: $u['username'];
        echo '<div class="ccard invite" onclick="newMyCard(' . json_encode($own) . ',' . json_encode((string) $u['email']) . ')">'
            . '<div class="chead"><div class="avatar">' . icon('plus', 16) . '</div>'
            . '<div class="cname"><strong>Meine Karte anlegen</strong><small>Eigene E-Mail, Telefon &amp; Co. hinterlegen</small></div></div></div>';
    }
    ...
    // Mitglieder der Plattform: direkt Chat starten (nur für aktive Konten)
    if ($u['status'] !== 'pending') {
        ...
        if ($members) {
            ...
            foreach ($members as $m) {
                ...
                echo '<div class="chead"><div class="avatar">' . h(strtoupper(mb_substr((string) $lbl, 0, 1))) . '</div>';
                echo '<div class="cname"><strong>' . h($lbl) . '</strong><small>@' . h($m['username']) . '</small></div>';
                echo '<span class="cacts">' . icon('send', 15) . '</span></div></a>';
            }
            echo '</div>';
        }
    }
    ...
    echo '<input type="hidden" name="id" value=""><input type="hidden" name="me" value=""><input type="hidden" name="back" value="">';
    echo '<div class="field"><label>Name</label><input class="input" name="name" required></div>';
    echo '<div class="row"><div class="field"><label>E-Mail</label><input class="input" type="email" name="email"></div>';
    echo '<div class="field"><label>Telefon</label><input class="input" type="tel" name="phone"></div></div>';
    echo '<div class="row"><div class="field"><label>Geburtstag</label><input class="input" type="date" name="birthday"></div>';
    echo '<div class="field"><label>Adresse</label><input class="input" name="address"></div></div>';
...
function render_settings(array $u): void {
    layout_topbar('Profil', 'Meine Karte, Konten & Aussehen',
        '<a class="btn ghost" href="?action=logout">' . icon('logout', 16) . ' Abmelden</a>');
    ...
    echo '<span class="badge ' . ($u['role'] === 'admin' ? 'admin' : h($u['status'])) . '">' . h($roleLbl) . '</span>';
    echo '</div>';

    /* Meine Karte: eigener Kontakt – hier und in Kontakte pflegbar */
    $mine = null;
    foreach (db_all('SELECT * FROM contacts WHERE user_id=?', [$u['id']]) as $row) {
        $c = dec_row($row['enc'], mk());
        if ($c && (int) ($c['me'] ?? 0) === 1) {
    ...
    echo '<div class="section-h">' . icon('user') . ' Meine Karte</div><div class="panel" style="max-width:560px">';
    echo '<form method="post" action="?app=contacts&action=save">' . csrf_field();
    echo '<input type="hidden" name="id" value="' . ($mine['id'] ?? '') . '">';
    echo '<input type="hidden" name="me" value="1"><input type="hidden" name="back" value="profile">';
    ...
    echo '<div class="field"><label>Notiz</label><textarea name="note" style="min-height:56px">' . h($mine['note'] ?? '') . '</textarea></div>';
    ...
    echo '<div class="section-h">' . icon('db') . ' Speicher</div><div class="panel" style="max-width:560px">';
    ...
    /* Konto */
    echo '<div class="section-h">' . icon('cog') . ' Konto</div><div class="panel" style="max-width:560px">';
    ...
    /* Aussehen */
    echo '<div class="section-h">' . icon('sun') . ' Aussehen</div><div class="panel" style="max-width:560px">';
    ...
    /* Passwort */
    echo '<div class="section-h">' . icon('key') . ' Passwort ändern</div><div class="panel" style="max-width:560px">';
    ...
    /* Mail-Konten */
    $accts = mail_accounts($u);
    echo '<div class="section-h">' . icon('mail') . ' Externe Mail-Konten</div><div class="panel" style="max-width:560px">';
    if ($accts) {
        foreach ($accts as $a) {
    ...
    /* Verwaltung: fuer Administratoren als Profil-Abschnitt */
    if ($u['role'] === 'admin') {
        echo '<div class="section-h">' . icon('shield') . ' Verwaltung</div>';
        admin_sections($u);
    }

    /* System */
    echo '<div class="section-h">' . icon('db') . ' System</div><div class="panel" style="max-width:560px">';
...
// Seite rendern
// Lesezeichen leben auf der Startseite, die Verwaltung im Profil –
// beide vor jeglicher Ausgabe umleiten.
if ($nxApp === 'bookmarks') {
    redirect(url('home'));
}
if ($nxApp === 'admin') {
