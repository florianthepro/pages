...
function serve_asset(string $which): void {
    ...
    if ($which === 'manifest') {
        ...
        echo json_encode([
            ...
            'icons' => [['src' => '?asset=icon', 'sizes' => '512x512', 'type' => 'image/png', 'purpose' => 'any']],
...
.shell{display:grid;grid-template-columns:72px 1fr;min-height:100vh;min-height:100dvh}
...
.ric{position:relative;width:58px;padding:7px 0 5px;border-radius:12px;display:flex;flex-direction:column;align-items:center;gap:3px;color:var(--muted);transition:background .12s,color .12s;flex:none}
.ric small{font-size:9px;font-weight:600;letter-spacing:.2px;line-height:1;max-width:56px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
 ...
 * ------------------------------------------------------------------ */
(function(){
  const chatIc=document.getElementById('ricChat');
  if(!chatIc)return;
  ...
  function ping(){
    ...
    fetch('?action=ping').then(r=>r.json()).then(d=>{
      ...
      const av=document.querySelector('.ravatar');
      if(av)setBadge(av,d.tickets||0,'warn');
...
/* Als App installieren (PWA): Android bekommt den echten Install-
   Prompt, iOS eine kurze Anleitung. */
let nxPwa=null;
window.addEventListener('beforeinstallprompt',e=>{
  e.preventDefault();nxPwa=e;
  const c=document.getElementById('pwaCard');
  if(c&&!localStorage.getItem('nx_pwa_hide'))c.hidden=false;
});
function pwaInstall(){
  if(nxPwa){nxPwa.prompt();nxPwa=null;const c=document.getElementById('pwaCard');if(c)c.hidden=true;}
  else{const p=document.getElementById('pwaIos');if(p)p.hidden=false;}
}
function pwaHide(){localStorage.setItem('nx_pwa_hide','1');const c=document.getElementById('pwaCard');if(c)c.hidden=true;}
(function(){
  const c=document.getElementById('pwaCard');if(!c)return;
  const standalone=window.matchMedia('(display-mode: standalone)').matches||navigator.standalone;
  const ios=/iphone|ipad|ipod/i.test(navigator.userAgent);
  if(ios&&!standalone&&!localStorage.getItem('nx_pwa_hide'))c.hidden=false;
})();
...
function layout_head(array $user, string $activeApp): void {
    ...
    echo '<link rel="manifest" href="?asset=manifest"><link rel="apple-touch-icon" href="?asset=icon">';
    ...
    foreach (nx_groups() as $grp) {
        ...
        foreach ($items as $id) {
            ...
            echo '<a class="ric' . ($id === $activeApp ? ' active' : '') . '"' . ($id === 'chat' ? ' id="ricChat"' : '')
                . ' href="' . url($id) . '" title="' . h($a['name']) . '">'
                . icon($a['icon'], 20) . '<small>' . h($a['name']) . '</small>' . $badge . '</a>';
        }
    }
    echo '</nav>';
    $pbadge = '';
...
function render_home(array $u): void {
    ...
    if (can_access($u, 'bookmarks')) {
        ...
        echo '<a class="bmtile add" href="?app=home&view=bm"><span class="tico">' . icon('plus', 15) . '</span>'
            . '<span class="bmtxt"><strong>Link hinzufügen</strong></span></a>';
        echo '</div>';
    }
    ...
    /* Als App installieren: erscheint nur, wenn moeglich und nicht
       bereits installiert (JS blendet ein; iOS bekommt eine Anleitung). */
    echo '<div class="panel pwa" id="pwaCard" hidden>';
    echo '<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">';
    echo '<span class="tico" style="width:38px;height:38px;--tc:var(--accent)">' . icon('download', 18) . '</span>';
    echo '<div style="flex:1;min-width:180px"><strong>Als App installieren</strong>';
    echo '<p style="color:var(--muted);font-size:12.5px;margin:0">' . NX_NAME . ' auf dem Startbildschirm – wie eine native App.</p></div>';
    echo '<div style="display:flex;gap:8px"><button class="btn" onclick="pwaInstall()">Installieren</button>';
    echo '<button class="btn ghost" onclick="pwaHide()">Später</button></div></div>';
    echo '<p id="pwaIos" hidden style="color:var(--muted);font-size:13px;margin-top:10px">'
        . 'Auf dem iPhone/iPad: unten <b>Teilen</b> antippen und dann <b>„Zum Home-Bildschirm"</b> wählen.</p>';
    echo '</div>';
...
function render_settings(array $u): void {
    ...
    if ($u['role'] === 'admin') {
        $open = tickets_open_count();
        echo '<div class="mlist">';
        echo $row('?app=settings&view=admin', 'shield', '#c25a5a', 'Verwaltung',
