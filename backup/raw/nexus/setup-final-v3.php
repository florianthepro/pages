...
.input,textarea,select{width:100%;min-width:0;max-width:100%;padding:10px 13px;border-radius:11px;border:1px solid var(--line);background:var(--bg2);color:var(--txt);transition:border-color .12s,box-shadow .12s;outline:none;font-size:13.5px}
...
.tile{position:relative;background:var(--panel);border:1px solid var(--line);border-radius:var(--radius);padding:15px;cursor:pointer;transition:transform .14s,box-shadow .14s,border-color .14s;display:flex;flex-direction:column;gap:10px;min-height:100px;box-shadow:var(--shadow-sm)}
.tile:hover{border-color:color-mix(in srgb,var(--tc,var(--accent)) 45%,var(--line));box-shadow:var(--shadow);transform:translateY(-2px)}
.tile .tico{width:38px;height:38px;border-radius:11px;display:grid;place-items:center;color:var(--tc,var(--accent));background:color-mix(in srgb,var(--tc,var(--accent)) 14%,transparent)}
.tile h3{font-size:14.5px;font-weight:650}
.tile p{color:var(--muted);font-size:12px;margin-top:-5px;line-height:1.4}
.section-h{display:flex;align-items:center;gap:9px;margin:22px 0 12px;font-size:11.5px;font-weight:600;color:var(--muted2);text-transform:uppercase;letter-spacing:.7px}
...
.ccard{background:var(--panel);border:1px solid var(--line);border-radius:var(--radius);padding:15px;box-shadow:var(--shadow-sm);display:flex;flex-direction:column;gap:10px}
.ccard.me{border-color:color-mix(in srgb,var(--accent) 40%,var(--line));background:color-mix(in srgb,var(--accent) 4%,var(--panel))}
.ccard.invite{cursor:pointer;border-style:dashed;box-shadow:none;color:var(--muted)}
...
// sigChain serialisiert die Verarbeitung; iceQueue puffert Kandidaten,
// die vor der Remote-Description eintreffen (sonst gingen sie verloren,
// da der Server zugestellte Signale löscht).
let pc=null,localStream=null,nxRing=false,sigChain=Promise.resolve(),iceQueue=[],remoteSet=false,nxFast=null;
...
function newPC(){
  ...
  p.onconnectionstatechange=()=>{if(p.connectionState==='failed')endCall();};
  return p;
}
async function flushIce(){
  remoteSet=true;
  while(iceQueue.length){const c=iceQueue.shift();try{await pc.addIceCandidate(c);}catch(e){}}
}
...
async function startCall(){
  ...
  try{
    ...
    await callMedia();pc=newPC();remoteSet=false;iceQueue=[];
...
async function onSig(s){
  ...
  try{
    if(s.type==='offer'){
      ...
      await flushIce();callFast(true);
    }else if(s.type==='answer'&&pc){
      await pc.setRemoteDescription(s.sdp);
      await flushIce();
    }else if(s.type==='ice'){
      if(!pc||!remoteSet){iceQueue.push(s.c);}
      else{try{await pc.addIceCandidate(s.c);}catch(e){}}
    }else if(s.type==='bye'){
      endCall(false);
    }
  }catch(e){}
...
function endCall(notify){
  ...
  if(notify&&nxPeer>0&&pc)sendSig({type:'bye'});
  if(pc){pc.close();pc=null;}
  if(localStream){localStream.getTracks().forEach(t=>t.stop());localStream=null;}
  remoteSet=false;iceQueue=[];callFast(false);
  document.getElementById('callbox')?.classList.remove('open');
}
window.addEventListener('beforeunload',()=>{if(pc)endCall();});
NXJS;
}
