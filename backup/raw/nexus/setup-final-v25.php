...
var nxStoreCat='default';
function storeApply(){var s=document.getElementById('storeSearch');var q=(s&&s.value||'').toLowerCase().trim();var adv=document.getElementById('storeAdv');
 document.querySelectorAll('#storeGrid .wacard').forEach(function(c){var show=q?(c.dataset.name.indexOf(q)>=0):(c.dataset.grp===nxStoreCat);c.style.display=show?'':'none';});
 if(adv)adv.style.display=(!q&&nxStoreCat==='adv')?'':'none';
 var grid=document.getElementById('storeGrid');if(grid)grid.style.display=(!q&&nxStoreCat==='adv')?'none':'';
 var note=document.getElementById('storeNote');if(note)note.style.display=q?'none':'';}
function storeCat(cat){nxStoreCat=cat;document.querySelectorAll('#storeChips .chip-btn').forEach(function(b){b.classList.toggle('on',b.dataset.cat===cat);});var s=document.getElementById('storeSearch');if(s)s.value='';storeApply();}
function storeSearchAll(){storeApply();}
function storeFilter(){storeApply();}
...
if(document.getElementById('storeGrid')){storeApply();}
...
function render_store(array $u): void {
    ...
    echo '<div class="store-intro">' . icon('grid', 18) . '<div><strong>So fügst du Apps hinzu</strong>'
        . '<p>Wähle eine Kategorie, dann bei einer App auf <b>+ Hinzufügen</b> tippen – sie erscheint auf deiner Startseite.</p></div></div>';

    // Suche
    echo '<div class="store-bar"><input class="input" id="storeSearch" placeholder="App suchen…" oninput="storeSearchAll()"></div>';

    // Kategorien: Standard + Web-Kategorien + Erweitert
    $cats = ['default' => 'Standard'];
    foreach (nx_webapps() as $w) { $cats[$w['cat']] = $w['cat']; }
    $cats['adv'] = 'Erweitert';
    echo '<div class="store-chips" id="storeChips">';
    $firstCat = true;
    foreach ($cats as $ck => $cl) {
        echo '<button type="button" class="chip-btn' . ($firstCat ? ' on' : '') . '" data-cat="' . h($ck) . '" onclick="storeCat(this.dataset.cat)">' . h($cl) . '</button>';
        $firstCat = false;
    }
    echo '</div>';

    // Eine gemeinsame Grid; Karten tragen data-cat, JS zeigt nur die aktive
    echo '<div class="cgrid" id="storeGrid">';
    foreach (nx_store_builtins() as $id) {
    ...
    foreach (nx_webapps() as $wid => $w) {
        $id = 'web:' . $wid;
        echo $card($id, $w['name'], 'Läuft in Nexus', nx_wa_color($w['name']), h(strtoupper(mb_substr($w['name'], 0, 1))), in_array($id, $inst, true), h($w['cat']));
    }
    echo '</div>';

    // Erweitert (Lesezeichen) – eigener Bereich, nur bei aktiver Kategorie 'adv'
    echo '<div class="mlist settings-w" id="storeAdv" style="display:none">';
    ...
    echo '<p class="note-line" id="storeNote" style="max-width:560px;margin-top:12px">„Standard" sind die nativen Nexus-Apps. Die übrigen Kategorien sind Dienste mit offiziellem Einbett-Player – sie laufen in Nexus. Seiten ohne Einbettung gehen unter „Erweitert" als Lesezeichen.</p>';
}
