...
function nx_webapps(): array {
    return [
        ...
        'gmaps'          => ['name'=>'Google Maps','cat'=>'Karten','embed'=>true,'frame'=>'https://www.google.com','ratio'=>'full','special'=>'gmaps','hint'=>'Ort oder Adresse'],
        ...
        'gdocs'          => ['name'=>'Google Docs','cat'=>'Dokumente','embed'=>true,'frame'=>'https://docs.google.com','ratio'=>'full','special'=>'gdoc','hint'=>'Google-Docs/Sheets/Slides-Link'],
        'gcal'           => ['name'=>'Google Kalender','cat'=>'Dokumente','embed'=>true,'frame'=>'https://calendar.google.com','ratio'=>'full','special'=>'gcal','hint'=>'Google-Kalender src oder Einbett-Link'],
        'gforms'         => ['name'=>'Google Forms','cat'=>'Dokumente','embed'=>true,'frame'=>'https://docs.google.com','ratio'=>'full','rx'=>'(https?://docs\\.google\\.com/forms/[^\\s]+)','tpl'=>'%s','hint'=>'Google-Forms-Link'],
...
.store-bar{display:flex;gap:10px;align-items:center;flex-wrap:wrap;max-width:560px;margin-bottom:14px}
...
function storeFilter(){var s=document.getElementById('storeSearch'),d=document.getElementById('storeDefaultOnly');var q=(s&&s.value||'').toLowerCase().trim();var defOnly=d&&d.checked;
document.querySelectorAll('.wacard').forEach(function(c){var ok=(!q||c.dataset.name.indexOf(q)>=0)&&(!defOnly||c.dataset.grp==='default');c.style.display=ok?'':'none';});
document.querySelectorAll('[data-store-group]').forEach(function(g){var grp=g.getAttribute('data-grp');var advHide=defOnly&&grp==='adv';var any=Array.from(g.querySelectorAll('.wacard')).some(function(c){return c.style.display!=='none';})&&!advHide;if(g.querySelectorAll('.wacard').length===0){any=!advHide&&!q;}var h=g.previousElementSibling;if(h&&h.classList.contains('section-h'))h.style.display=any?'':'none';g.style.display=any?'':'none';});}
...
function render_store(array $u): void {
    ...
    $card = static function (string $id, string $name, string $desc, string $color, string $ico, bool $on, string $grp) use ($btn): string {
        $out  = '<div class="ccard wacard" data-grp="' . $grp . '" data-name="' . h(mb_strtolower($name)) . '"><div class="chead">';
    ...
    // Suche + Filter
    echo '<div class="store-bar">';
    echo '<input class="input" id="storeSearch" placeholder="App suchen…" oninput="storeFilter()">';
    echo '<label class="store-chk"><input type="checkbox" id="storeDefaultOnly" onchange="storeFilter()"> Nur Standard-Apps</label>';
    echo '</div>';

    // Standard-Apps (nativ von Nexus)
    echo '<div class="section-h" data-grp-h="default">' . icon('grid') . ' Standard-Apps</div><div class="cgrid" data-store-group>';
    foreach (nx_store_builtins() as $id) {
        if (!can_access($u, $id)) { continue; }
        $a = $apps[$id];
        echo $card($id, $a['name'], $a['desc'], $a['color'], icon($a['icon'], 17), in_array($id, $inst, true), 'default');
    }
    echo '</div>';

    // Apps (nativ eingebettete Web-Apps) nach Kategorie
    $byCat = [];
    foreach (nx_webapps() as $wid => $w) { $byCat[$w['cat']]['web:' . $wid] = $w; }
    ksort($byCat);
    foreach ($byCat as $cat => $list) {
        echo '<div class="section-h" data-grp-h="apps">' . icon('download') . ' ' . h($cat) . '</div><div class="cgrid" data-store-group>';
        foreach ($list as $id => $w) {
            echo $card($id, $w['name'], 'Läuft in Nexus', nx_wa_color($w['name']), h(strtoupper(mb_substr($w['name'], 0, 1))), in_array($id, $inst, true), 'apps');
        }
        echo '</div>';
    }

    // Erweitert: Lesezeichen
    echo '<div class="section-h" data-grp-h="adv">' . icon('link') . ' Erweitert</div>';
    echo '<div class="mlist" style="max-width:560px" data-store-group data-grp="adv">';
    echo '<a class="mrow" href="?app=store&view=bm"><span class="mic" style="--mc:#8a7fb0">' . icon('link', 16) . '</span>';
    echo '<span class="mlabel"><strong>Lesezeichen</strong><small style="color:var(--muted);font-weight:400">Beliebige Website als Kachel</small></span><span class="chev">' . icon('chevR', 15) . '</span></a></div>';

    echo '<p class="note-line" style="max-width:560px;margin-top:12px">Standard-Apps sind nativ von Nexus. „Apps" sind Dienste mit offiziellem Einbett-Player – sie laufen in Nexus. '
        . 'Seiten ohne Einbett-Möglichkeit lassen sich unter „Erweitert" als Lesezeichen hinzufügen.</p>';
}
...
function render_settings(array $u): void {
    ...
    if ($view === 'account') {
        ...
        // Anmeldedaten
        echo '<div class="section-h">' . icon('cog') . ' Anmeldedaten</div><div class="panel" style="max-width:560px">';
        ...
        // Passwort
        echo '<div class="section-h">' . icon('key') . ' Passwort</div><div class="panel" style="max-width:560px">';
    ...
    if ($view === 'connections') {
        ...
        // Mail-Konten
        echo '<div class="section-h">' . icon('mail') . ' Mail-Konten</div><div class="panel" style="max-width:560px">';
        $accts = mail_accounts($u);
        if ($accts) {
            foreach ($accts as $a) {
    ...
    /* --- SPEICHER & SYSTEM --- */
    if ($view === 'system') {
        layout_topbar('Speicher & System', '', '', $back);
        ...
        echo '<div class="section-h">' . icon('cog') . ' System</div><div class="panel" style="max-width:560px">';
    ...
    /* --- HAUPTSEITE: Kopf + wenige, grosse Menuepunkte --- */
    layout_topbar('Einstellungen', '');
    ...
    $row = static function (string $href, string $ic, string $color, string $label, string $sub, string $val = ''): string {
        $out = '<a class="mrow" href="' . $href . '"><span class="mic" style="--mc:' . $color . '">' . icon($ic, 16) . '</span>';
        $out .= '<span class="mlabel"><strong>' . $label . '</strong><small style="color:var(--muted);font-weight:400">' . $sub . '</small></span>';
        if ($val !== '') { $out .= '<span class="mval">' . $val . '</span>'; }
        return $out . '<span class="chev">' . icon('chevR', 15) . '</span></a>';
    };

    echo '<div class="mlist">';
    echo $row('?app=settings&view=account', 'user', '#4d7ea8', 'Konto', 'Meine Karte, Anmeldedaten & Passwort');
    echo $row('?app=settings&view=appearance', 'sun', '#b3893f', 'Aussehen', 'Theme & Akzentfarbe', $u['theme'] === 'dark' ? 'Dunkel' : 'Hell');
    echo $row('?app=settings&view=connections', 'mail', '#4a8ca0', 'Verbindungen', 'Mail, Kalender & Kontakte', (string) (count(mail_accounts($u)) + count(nx_sync_accounts($u))));
    echo $row('?app=settings&view=system', 'db', '#4a9d6f', 'Speicher & System', 'Belegung & Info');
    echo '</div>';

    if ($u['role'] === 'admin') {
        $open = tickets_open_count();
        echo '<div class="mlist">';
        echo $row('?app=settings&view=admin', 'shield', '#c25a5a', 'Verwaltung', 'Nutzer, Freischaltung & Quota',
            $open > 0 ? '<span class="nav-badge warn">' . $open . '</span>' : '');
        echo '</div>';
    }

    echo '<div class="mlist">';
    echo '<a class="mrow danger" href="?action=logout"><span class="mic">' . icon('logout', 16) . '</span><span class="mlabel"><strong>Abmelden</strong></span></a>';
    echo '</div>';
}
