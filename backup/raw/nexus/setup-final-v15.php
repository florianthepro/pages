...
/** Fertige, wirklich einbettbare Web-Apps (erlauben Framing). */
function nx_webapps(): array {
    return [
        'youtube' => ['name' => 'YouTube', 'color' => '#c0392b', 'icon' => 'video', 'kind' => 'player',
            'frame' => 'https://www.youtube-nocookie.com',
            'desc'  => 'YouTube-Videos direkt in Nexus abspielen'],
        'osm'     => ['name' => 'Karte', 'color' => '#4a9d6f', 'icon' => 'pin', 'kind' => 'embed',
            'url'   => 'https://www.openstreetmap.org/export/embed.html?layer=mapnik',
            'frame' => 'https://www.openstreetmap.org',
            'desc'  => 'OpenStreetMap – Karte einbetten'],
        'wikipedia' => ['name' => 'Wikipedia', 'color' => '#555', 'icon' => 'note', 'kind' => 'embed',
            'url'   => 'https://de.m.wikipedia.org/wiki/Wikipedia:Hauptseite',
            'frame' => 'https://de.m.wikipedia.org',
            'desc'  => 'Wikipedia mobil einbetten'],
    ];
}

/** Installierte App-IDs des Nutzers (built-in ids + 'web:ID'). */
...
function render_home(array $u): void {
    ...
    foreach (nx_webapps() as $wid => $w) {
        ...
        echo '<span class="lico" style="--tc:' . h($w['color']) . '">' . icon($w['icon'], 22) . '</span>';
...
function render_store(array $u): void {
    layout_topbar('App-Store', 'Apps hinzufügen & entfernen', '', url('home'));
    $inst = nx_installed($u);
    $apps = nx_apps();
    ...
    $btn = static function (string $id, bool $on): string {
        if ($on) {
            return '<form method="post" action="?app=store&action=remove" style="margin:0">' . csrf_field()
                . '<input type="hidden" name="id" value="' . h($id) . '"><button class="btn ghost sm">Entfernen</button></form>';
        }
        return '<form method="post" action="?app=store&action=install" style="margin:0">' . csrf_field()
            . '<input type="hidden" name="id" value="' . h($id) . '"><button class="btn sm">Hinzufügen</button></form>';
    };

    echo '<div class="section-h">' . icon('grid') . ' Nexus-Apps</div><div class="cgrid">';
    foreach (nx_store_builtins() as $id) {
        $a = $apps[$id];
        if (!can_access($u, $id)) {
            continue;
        }
        echo '<div class="ccard"><div class="chead">';
        echo '<span class="avatar" style="background:color-mix(in srgb,' . h($a['color']) . ' 16%,transparent);color:' . h($a['color']) . '">' . icon($a['icon'], 17) . '</span>';
        echo '<span class="cname"><strong>' . h($a['name']) . '</strong><small>' . h($a['desc']) . '</small></span></div>';
        echo '<div class="cbody"><div class="store-act">' . $btn($id, in_array($id, $inst, true)) . '</div></div></div>';
    }
    echo '</div>';

    echo '<div class="section-h">' . icon('download') . ' Web-Apps (nativ eingebettet)</div><div class="cgrid">';
    foreach (nx_webapps() as $wid => $w) {
        $id = 'web:' . $wid;
        echo '<div class="ccard"><div class="chead">';
        echo '<span class="avatar" style="background:color-mix(in srgb,' . h($w['color']) . ' 16%,transparent);color:' . h($w['color']) . '">' . icon($w['icon'], 17) . '</span>';
        echo '<span class="cname"><strong>' . h($w['name']) . '</strong><small>' . h($w['desc']) . '</small></span></div>';
        echo '<div class="cbody"><div class="store-act">' . $btn($id, in_array($id, $inst, true)) . '</div></div></div>';
    }
    echo '</div>';
    echo '<p class="note-line" style="max-width:560px">Es sind nur Apps aufgeführt, die sich sauber und sicher in Nexus einbetten lassen. '
        . 'Seiten, die das Einbetten verbieten (z. B. die volle YouTube-Seite), sind bewusst nicht dabei.</p>';
}
