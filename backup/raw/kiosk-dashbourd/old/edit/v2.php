...
function setTab(t){
tab=(t==="page"||t==="widget"||t==="schedule")?t:"widget";
["page","widget","schedule"].forEach(x=>{let b=document.getElementById("t"+x[0].toUpperCase()+x.slice(1));if(b)b.classList.toggle("active",x===tab);});
renderSide();
}
...
let row=document.createElement("div");row.className="pg"+(i===pi?" active":"");row.onclick=()=>{pi=i;selectedIds=[];renderAll();};
...
let acts=document.createElement("div");acts.className="acts";
let bUp=miniBtn("▲",e=>{e.stopPropagation();movePage(i,-1);});
let bDn=miniBtn("▼",e=>{e.stopPropagation();movePage(i,1);});
let bRen=miniBtn("✎",e=>{e.stopPropagation();let nn=prompt("Page name:",p.name);if(nn!=null){nn=String(nn).trim();if(nn)p.name=nn;renderPages();renderCanvas();}});
let bDup=miniBtn("⎘",e=>{e.stopPropagation();dupPage(i);});
let bDel=miniBtn("✕",e=>{e.stopPropagation();delPage(i);});
acts.appendChild(bUp);acts.appendChild(bDn);acts.appendChild(bRen);acts.appendChild(bDup);acts.appendChild(bDel);
...
d.ondblclick=e=>{e.stopPropagation();selectedIds=[w.id];setTab("widget");renderAll();};
...
function addRule(){
data.schedules.push({id:uid(),name:"Rule "+(data.schedules.length+1),enabled:true,from:"08:00",to:"18:00",weekdays:[1,2,3,4,5],page:getPage().id,override:{rotationSpeed:null,background:null,widgets:{}}});
setTab("schedule");
renderAll();
}
...
let br=document.createElement("div");br.className="btnRow";
br.appendChild(btn("+ Rule","",()=>addRule()));
f.appendChild(br);
let rules=data.schedules.map((r,idx)=>({r,idx})).filter(x=>(x.r.page||"")===(pid||""));
if(!rules.length){let t=document.createElement("div");t.className="small";t.textContent="No rules for this page.";f.appendChild(t);elPanel.appendChild(f);return;}
rules.forEach(x=>{
let r=x.r,idx=x.idx;
let rr=fieldset("");rr.style.borderStyle="dashed";
...
let a=document.createElement("div");a.appendChild(label("Name"));a.appendChild(input("text",r.name||"",v=>{r.name=v;renderSide();}));
let b=document.createElement("div");b.appendChild(label("On"));b.appendChild(checkbox(r.enabled,v=>{r.enabled=v;renderSide();}));
top.appendChild(a);top.appendChild(b);rr.appendChild(top);
...
tm.appendChild(fr);tm.appendChild(to);rr.appendChild(tm);
...
let l=document.createElement("label");l.style.display="flex";l.style.gap="8px";l.style.alignItems="center";l.style.margin="0";l.appendChild(c);let t=document.createElement("span");t.textContent=n;l.appendChild(t);
d.appendChild(l);wd.appendChild(d);
});
rr.appendChild(wd);
let rb=document.createElement("div");rb.className="btnRow";
rb.appendChild(btn("Up","alt",()=>moveRule(idx,-1)));
rb.appendChild(btn("Down","alt",()=>moveRule(idx,1)));
rb.appendChild(btn("Copy","alt",()=>{let c=deepClone(r);c.id=uid();c.name=(c.name||"Rule")+" Copy";data.schedules.splice(idx+1,0,c);renderSide();}));
rb.appendChild(btn("Del","danger",()=>{data.schedules.splice(idx,1);renderSide();}));
rr.appendChild(rb);
f.appendChild(rr);
});
