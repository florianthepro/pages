...
:: Pfad des Skript-Ordners
set "SDIR=%~dp0"

:: Erzeuge webmanager.ps1 (überschreibt vorhandenes)
> "%SDIR%webmanager.ps1" (
  ...
  echo ^    }
  echo }
  echo # Standard-Download-URLs (anpassbar)
  ...
  echo ^    }
  echo ^    # Versuche Service-Install wenn httpd.exe vorhanden
  ...
  echo ^    } else { Write-Host "httpd.exe nicht gefunden in $APACHE_HOME" -ForegroundColor Yellow }
  echo }
  echo # Lade Config und initialen Status
  echo Load-Config
  echo Refresh-Status
...
:: Starte das PowerShell-Skript (ExecutionPolicy bypass)
powershell -NoProfile -ExecutionPolicy Bypass -File "%SDIR%webmanager.ps1"
endlocal
