...
if(PHP_VERSION_ID<80000){http_response_code(500);header('Content-Type: text/plain; charset=utf-8');exit('kiosk-dashboard benoetigt PHP 8.0 oder neuer (aktuell: '.PHP_VERSION.').');}
///////////////////////
$kioskdashboard_title='Kiosk Dashboard';
$kioskdashboard_theme='auto'; #'light' (fix hell) | 'dark' (fix dunkel) | 'auto' (dynamisch, folgt System)
$kioskdashboard_datadir='kiosk-dashboard-data'; #Datenordner: name relativ zur Instanz, absoluter Pfad auch moeglich
$kioskdashboard_allow_private_fetch=false; #true erlaubt der fetchHtml-Proxy Zugriffe auf private/interne IPs (SSRF-Risiko)
$kioskdashboard_icon='https://raw.githubusercontent.com/florianthepro/pages/main/content/media/kiosk-dashboard/index.svg';
$kioskdashboard_basedir=dirname((string)($_SERVER['SCRIPT_FILENAME']??__FILE__));
if(!preg_match('~^([/\\\\]|[A-Za-z]:)~',$kioskdashboard_datadir))$kioskdashboard_datadir=$kioskdashboard_basedir.DIRECTORY_SEPARATOR.$kioskdashboard_datadir;
$kioskdashboard_jsondir=$kioskdashboard_datadir.DIRECTORY_SEPARATOR.'data.json';
...
index: "https://raw.githubusercontent.com/florianthepro/pages/main/content/routes/kiosk-dashboard/index.php"
edit: "https://raw.githubusercontent.com/florianthepro/pages/main/content/routes/kiosk-dashboard/edit.php"
YAML;
///////////////////////
if(!is_dir($kioskdashboard_datadir))@mkdir($kioskdashboard_datadir,0770,true);
if(is_dir($kioskdashboard_datadir)){
if(!is_file($kioskdashboard_datadir.DIRECTORY_SEPARATOR.'.htaccess'))@file_put_contents($kioskdashboard_datadir.DIRECTORY_SEPARATOR.'.htaccess',"<IfModule mod_authz_core.c>\nRequire all denied\n</IfModule>\n<IfModule !mod_authz_core.c>\nDeny from all\n</IfModule>\n");
if(!is_file($kioskdashboard_datadir.DIRECTORY_SEPARATOR.'index.php'))@file_put_contents($kioskdashboard_datadir.DIRECTORY_SEPARATOR.'index.php',"<?php http_response_code(403); exit;");
}
