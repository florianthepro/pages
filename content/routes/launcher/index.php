...
function readDefaultLinks(){
  ...
  DEFAULT_LINKS.forEach(l => {
    ...
    result[g].push({ title: l.title || l.url, url: l.url, iconUrl: l.icon || null, src: 'yaml' });
  });
  return result;
}
/* Die YAML ist die Vorgabe, die eigene Anordnung bleibt trotzdem stehen:
   gespeicherte Kacheln behalten ihren Platz, neue Zeilen aus der YAML kommen
   dazu, geloeschte fallen weg. Selbst angelegte oder im Editor bearbeitete
   Kacheln (src != 'yaml') fasst die YAML nicht mehr an. Ohne diesen Abgleich
   wuerde der gespeicherte Stand die YAML dauerhaft ueberdecken. */
function mergeLinks(saved, defaults){
  const byUrl = {};
  Object.keys(defaults).forEach(g => defaults[g].forEach(d => { byUrl[d.url] = d; }));
  const out = {}, seen = {};
  Object.keys(saved).forEach(g => {
    const keep = [];
    (saved[g] || []).forEach(it => {
      if (!it || !it.url) return;
      const d = byUrl[it.url];
      const kind = it.src || (d ? 'yaml' : 'user');   /* aelterer Stand ohne Markierung */
      if (kind === 'yaml') {
        if (!d) return;                               /* Zeile aus der YAML entfernt */
        seen[d.url] = true;
        keep.push({ title: d.title, url: d.url, iconUrl: d.iconUrl, src: 'yaml' });
      } else {
        keep.push(it);
      }
    });
    if (keep.length) out[g] = keep;
  });
  Object.keys(defaults).forEach(g => {
    defaults[g].forEach(d => {
      if (seen[d.url]) return;
      if (!out[g]) out[g] = [];
      out[g].push({ title: d.title, url: d.url, iconUrl: d.iconUrl, src: 'yaml' });
    });
  });
  return out;
}
function loadLinks(){
  const defaults = readDefaultLinks();
  const raw = getCookie(COOKIE_NAME);
  if (raw) {
    try { const p = JSON.parse(raw); if (p && typeof p === 'object') return mergeLinks(p, defaults); } catch(e){}
  }
  return defaults;
}
...
function editLink(gid, idx){
  ...
  /* bearbeitet = gehoert jetzt dem Nutzer, die YAML ueberschreibt es nicht mehr */
  allLinks[gid][idx] = { title: newTitle.trim(), url: newUrl.trim(), iconUrl: newIconUrl || null, src: 'user' };
...
function handleAdd(gid){
  ...
  allLinks[gid].push({ title: title.trim(), url: url.trim(), iconUrl: iconUrl || null, src: 'user' });
  saveLinks(allLinks); buildAllGrids();
}
...
buildAllGrids();
window.Launcher = {
  add(title, url, gid = 'general'){ if (!allLinks[gid]) allLinks[gid] = []; allLinks[gid].push({ title, url, iconUrl: null, src: 'user' }); saveLinks(allLinks); buildAllGrids(); },
  reset(){ resetLauncher(); },
