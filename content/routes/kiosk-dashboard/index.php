...
$kioskdashboard_allow_private_fetch=isset($kioskdashboard_allow_private_fetch)?(bool)$kioskdashboard_allow_private_fetch:false;
$allow_private_fetch=$kioskdashboard_allow_private_fetch;
...
function json_path($override=""){ if(is_string($override)&&$override!==""){return $override;} $p1=__DIR__."/data/data.json"; $p2=__DIR__."/data.json"; if(file_exists($p1))return $p1; return $p2; }
...
#Resolviert den Host und gibt NUR die gepruervten oeffentlichen IPs zurueck (oder false).
#Diese IPs werden anschliessend per CURLOPT_RESOLVE an curl gepinnt, damit curl NICHT
#erneut aufloesen kann (Schutz gegen DNS-Rebinding zwischen Pruefung und Abruf).
function resolveHostSafe($host,$allow_private_fetch){$ips=@gethostbynamel($host);if(!$ips)return false;$out=[];foreach($ips as $ip){if($allow_private_fetch){$out[]=$ip;continue;}if(filter_var($ip,FILTER_VALIDATE_IP,FILTER_FLAG_NO_PRIV_RANGE|FILTER_FLAG_NO_RES_RANGE))$out[]=$ip;else return false;}return $out?array_values(array_unique($out)):false;}
$path=json_path(isset($kioskdashboard_jsondir)&&is_string($kioskdashboard_jsondir)?$kioskdashboard_jsondir:"");
$data=$default;
...
$vetIps=resolveHostSafe($host,$allow_private_fetch);
if($vetIps===false){http_response_code(403);exit;}
#curl an die gepruervten IPs pinnen und Redirects abschalten -> curl kann nicht neu aufloesen (kein DNS-Rebinding, kein Redirect-Rebind).
...
$max=2*1024*1024;$buf="";
$ch=curl_init($url);
$copt=[CURLOPT_RETURNTRANSFER=>false,CURLOPT_FOLLOWLOCATION=>false,CURLOPT_MAXREDIRS=>0,CURLOPT_CONNECTTIMEOUT=>4,CURLOPT_TIMEOUT=>12,CURLOPT_USERAGENT=>"DashboardPlayer/1.0",CURLOPT_HTTPHEADER=>["Accept: text/html,application/xhtml+xml"],CURLOPT_RESOLVE=>$resolve,CURLOPT_WRITEFUNCTION=>function($ch,$data)use(&$buf,$max){$buf.=$data;return strlen($buf)>$max?0:strlen($data);}];
...
if(trim($title0)===""&&isset($kioskdashboard_title))$title0=(string)$kioskdashboard_title;
$icon=safe_href(($meta["icon"]??($meta["favicon"]??""))?:($data["pages"][0]["thumb"]??""));
if($icon===""&&isset($kioskdashboard_icon))$icon=safe_href((string)$kioskdashboard_icon);
?>
<!doctype html>
...
<style>
:root{--kd-bg:#e9edf2;}
:root[data-theme="dark"]{--kd-bg:#000;}
@media (prefers-color-scheme: dark){:root:not([data-theme="light"]):not([data-theme="dark"]){--kd-bg:#000;}}
html,body{margin:0;height:100%;background:var(--kd-bg);overflow:hidden;font-family:system-ui,Segoe UI,Arial}
#wrap{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:var(--kd-bg)}
