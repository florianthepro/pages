...
$path=(isset($kioskdashboard_jsondir)&&is_string($kioskdashboard_jsondir)&&$kioskdashboard_jsondir!=="")?$kioskdashboard_jsondir:(__DIR__."/data.json");
$allow_private_fetch=isset($kioskdashboard_allow_private_fetch)?(bool)$kioskdashboard_allow_private_fetch:false;
$kd_theme=(isset($kioskdashboard_theme)&&in_array($kioskdashboard_theme,['light','dark'],true))?$kioskdashboard_theme:'auto';
...
#Host aufloesen und NUR gepruervte oeffentliche IPs behalten; diese werden per CURLOPT_RESOLVE
#gepinnt, damit curl nicht erneut aufloesen kann (Schutz gegen DNS-Rebinding).
$resolveHost=function($h)use($allow_private_fetch){
$ips=@gethostbynamel($h);
if(!$ips)return false;
$out=[];
foreach($ips as $ip){
if($allow_private_fetch){$out[]=$ip;continue;}
if(filter_var($ip,FILTER_VALIDATE_IP,FILTER_FLAG_NO_PRIV_RANGE|FILTER_FLAG_NO_RES_RANGE))$out[]=$ip;
else return false;
}
return $out?array_values(array_unique($out)):false;
};
$vetIps=$resolveHost($host);
if($vetIps===false){http_response_code(403);exit;}
$reqPort=isset($u["port"])?(int)$u["port"]:($scheme==="https"?443:80);
$resolve=[];foreach($vetIps as $ip){foreach(array_unique([$reqPort,80,443]) as $rp){$resolve[]=$host.":".$rp.":".$ip;}}
$max=2*1024*1024;$buf="";
$ch=curl_init($url);
$copt=[CURLOPT_RETURNTRANSFER=>false,CURLOPT_FOLLOWLOCATION=>false,CURLOPT_MAXREDIRS=>0,CURLOPT_CONNECTTIMEOUT=>4,CURLOPT_TIMEOUT=>12,CURLOPT_USERAGENT=>"DashboardEditor/1.0",CURLOPT_HTTPHEADER=>["Accept: text/html,application/xhtml+xml"],CURLOPT_RESOLVE=>$resolve,CURLOPT_WRITEFUNCTION=>function($ch,$data)use(&$buf,$max){$buf.=$data;return strlen($buf)>$max?0:strlen($data);}];
if(defined("CURLPROTO_HTTP")&&defined("CURLPROTO_HTTPS")){$copt[CURLOPT_PROTOCOLS]=CURLPROTO_HTTP|CURLPROTO_HTTPS;}
...
?>
<!doctype html>
<html<?php echo $kd_theme==='light'?' data-theme="light"':($kd_theme==='dark'?' data-theme="dark"':''); ?>>
<head>
<meta charset=utf-8>
<meta name=viewport content="width=device-width,initial-scale=1">
<title><?php echo htmlspecialchars((isset($kioskdashboard_title)&&trim((string)$kioskdashboard_title)!=="")?((string)$kioskdashboard_title." · Editor"):"Dashboard Editor",ENT_QUOTES|ENT_SUBSTITUTE,"UTF-8"); ?></title>

<style>
:root{--bg:#e9edf2;--fg:#111827;--muted:#6b7280;--border:#d1d5db;--panel:#ffffff;--accent:#2563eb;--panel2:#f8fafc;--panelhead:#f9fafb;--hover:#f3f4f6;--btn:#111827;--btnfg:#ffffff;}
:root[data-theme="dark"]{--bg:#0b1020;--fg:#e5e7eb;--muted:#9ca3af;--border:#334155;--panel:#111827;--accent:#60a5fa;--panel2:#0f172a;--panelhead:#0f172a;--hover:#1f2937;--btn:#2563eb;--btnfg:#ffffff;}
@media (prefers-color-scheme: dark){:root:not([data-theme="light"]):not([data-theme="dark"]){--bg:#0b1020;--fg:#e5e7eb;--muted:#9ca3af;--border:#334155;--panel:#111827;--accent:#60a5fa;--panel2:#0f172a;--panelhead:#0f172a;--hover:#1f2937;--btn:#2563eb;--btnfg:#ffffff;}}
html,body{margin:0;height:100%;font-family:system-ui,Segoe UI,Arial;background:var(--bg)}
...
/* --- theme remap (light defaults == original colors; dark via data-theme / prefers-color-scheme) --- */
body{background:var(--bg)}
#panel{background:var(--panel);color:var(--fg)}
#centerTop{background:var(--panel2);border-bottom-color:var(--border)}
#centerTop .pill{background:var(--hover);border-color:var(--border);color:var(--fg)}
fieldset{border-color:var(--border)}
legend{color:var(--fg)}
label{color:var(--muted)}
input,select,textarea{background:var(--panel);color:var(--fg);border-color:var(--border)}
.small{color:var(--muted)}
.badge{background:var(--panel2);border-color:var(--border);color:var(--fg)}
.btnRow button,#modalFoot button,#editModalFoot button,#pagePopFoot button,.rpRow button{background:var(--btn);color:var(--btnfg);border-color:var(--btn)}
.btnRow button.alt,.rpRow button.alt{background:var(--panel);color:var(--fg);border-color:var(--border)}
.btnRow button.danger,.rpRow button.danger{background:#ef4444;color:#fff;border-color:#ef4444}
#modal,#editModal,#pagePop,#renamePop,#newPagePop,#weditPop{background:var(--panel);color:var(--fg);border-color:var(--border)}
#modalHead,#editModalHead{border-bottom-color:var(--border)}
#pagePopHead{background:var(--panelhead);color:var(--fg);border-bottom-color:var(--border)}
#modalBody,#editModalBody,#pagePopBody{color:var(--fg)}
#modalFoot,#editModalFoot,#pagePopFoot{border-top-color:var(--border)}
#modalCancel{background:var(--panel)!important;color:var(--fg)!important;border-color:var(--border)!important}
#renamePop input,#newPagePop input,#weditPop input,#weditPop select,#weditPop textarea{background:var(--panel);color:var(--fg);border-color:var(--border)}
#weditPop .lbl,#colorPop .lbl{color:var(--fg)}
#ctxMenu,#wctxMenu,#wctxHiddenMenu,#wctxAddMenu,#colorPop{background:var(--panel);border-color:var(--border)}
#ctxMenu button,#wctxMenu button,#wctxHiddenMenu button,#wctxAddMenu button{color:var(--fg)}
#ctxMenu button:hover,#wctxMenu button:hover,#wctxHiddenMenu button:hover,#wctxAddMenu button:hover{background:var(--hover)}
#ctxMenu .sep,#wctxMenu .sep{background:var(--border)}
#wctxHiddenMenu .empty{color:var(--muted)}
a{color:var(--accent)}
</style>
