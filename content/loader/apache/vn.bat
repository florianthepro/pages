...
:: Default Download-URLs (anpassbar)
...
if "%APACHE_INSTALLED%"=="1" (
  ...
  if "%A%"=="1" (
    if "%IS_ADMIN%"=="0" (
      echo Deinstallation benoetigt Administratorrechte. Starte die Batch als Administrator.
      pause
      goto main
    )
    if exist "%~dp0apache-uninstall.bat" (
      ...
      if /i "%DEL%"=="y" rd /s /q "%APACHE_HOME%" 2>nul & echo Verzeichnis geloescht.
    )
    call :refresh
    pause
    goto main
  ...
  if "%A%"=="1" (
    if exist "%~dp0apache-install.bat" (
      ...
      if /i "%DL%"=="y" (
        echo Lade Apache...
        powershell -NoProfile -Command "try{ Invoke-WebRequest -Uri '%DEFAULT_APACHE_URL%' -OutFile '%~dp0apache.zip' -UseBasicParsing; Write-Host 'OK' } catch { Write-Host 'ERR'; exit 1 }"
        if %errorlevel%==0 (
          if not exist "%APACHE_HOME%" mkdir "%APACHE_HOME%"
    ...
    if exist "%APACHE_HOME%\bin\httpd.exe" (
      ...
      ) else echo Hinweis: Service-Installation benoetigt Administratorrechte.
    )
    call :refresh
    pause
    goto main
...
if "%PHP_FOUND%"=="1" (
  ...
  if "%P%"=="1" (
    if exist "%~dp0php-install.bat" (
      ...
      if /i "%DL%"=="y" (
        echo Lade PHP...
        powershell -NoProfile -Command "try{ Invoke-WebRequest -Uri '%DEFAULT_PHP_URL%' -OutFile '%~dp0php.zip' -UseBasicParsing; Write-Host 'OK' } catch { Write-Host 'ERR'; exit 1 }"
        if %errorlevel%==0 (
          if not exist "%APACHE_HOME%\php" mkdir "%APACHE_HOME%\php"
          ...
          echo PHP entpackt. Bitte httpd.conf anpassen.
        ) else echo Download fehlgeschlagen.
      ) else echo Abgebrochen.
    )
    pause
  ...
  if "%APACHE_INSTALLED%"=="1" (
    if "%APACHE_SERVICE%"=="1" (
      if "%IS_ADMIN%"=="1" (
        ...
        net start "Apache2.4"
      ) else echo Service-Start benoetigt Administratorrechte.
    ) else (
      if exist "%APACHE_HOME%\bin\httpd.exe" (
  ...
  if "%APACHE_INSTALLED%"=="1" (
    if "%APACHE_SERVICE%"=="1" (
      if "%IS_ADMIN%"=="1" (
        ...
        net stop "Apache2.4"
      ) else echo Service-Stop benoetigt Administratorrechte.
    ) else (
      if exist "%APACHE_HOME%\bin\httpd.exe" (
