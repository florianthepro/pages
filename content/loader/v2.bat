...
:: apache-setup.bat - interaktives Menü für portable Apache+PHP
...
if "%AP_INSTALLED%"=="1" (
  ...
  echo [1] Apache  - Installieren (hersteller ZIP/EXE)
)
if "%AP_INSTALLED%"=="1" (
  if "%PHP_INSTALLED%"=="1" (
    echo [2] PHP     - Deinstallieren
  ) else (
    echo [2] PHP     - Installieren (hersteller ZIP/EXE)
  )
)
if "%AP_INSTALLED%"=="1" (
...
if "%AP_INSTALLED%"=="1" (
  ...
  echo Apache-Installation (hersteller ZIP/EXE)...
  ...
  if errorlevel 0 (
    ...
    rem Falls EXE: führe aus und warte
    start /wait "" "%TMPZIP%"
    del /f /q "%TMPZIP%" >nul 2>&1
...
if "%PHP_INSTALLED%"=="1" (
  ...
  echo PHP-Installation (hersteller ZIP/EXE)...
  set /p "PHP_URL=Gib die PHP-Download-URL (ZIP oder EXE) ein: "
  if "%PHP_URL%"=="" (
    echo Keine URL angegeben. Abbruch.
    pause
    goto menu
  ...
  echo Herunterladen...
  call :download_file "%PHP_URL%" "%TMPZIP%"
  if errorlevel 1 (
    echo Download fehlgeschlagen. Siehe Log: %LOG%
    pause
    goto menu
  )
  echo %PHP_URL% | findstr /i "\.zip$" >nul
  if errorlevel 0 (
  ...
  echo [%date% %time%] PHP-Install ausgefuehrt von %PHP_URL% >> "%LOG%"
  pause
  goto menu
)
...
echo [2] Aendere Installationspfad (verschiebt keine Dateien)
echo [3] Zeige Log
echo [0] Zurueck
set /p "C=Waehle: "
if "%C%"=="1" (
